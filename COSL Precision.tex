...
hand with slam interest, (c) bid \cl{2} (\emph{Transfer Stayman}) or
(d) raise no-trumps.
...
Responder's rebids after opener's \cl{2} \emph{Transfer Stayman} bid
are:
...
initiate $\beta$ and ask for the number of controls held (\excp{the
  0-3 scale is used after a \cl{3} or \di{3} response and the $4^+$
...
\di{1}, e.g., \cl{1}--(\emph{Pass})--\di{1}--(\emph{RHO doubles /
...
\hypertarget{2d} A \di{2} opening declares a three suited hand with a
...
responses are the artificial bids of \nt{2} or \di{3} which are both
forcing to game.
...
and no five-card suit. Note that the rebids after a \cl{3} response
...
Responsive doubles are ``on'' up to the level of \sp{3} and can be
...
\cl{2}/\di{2}) it shows both majors and over a major-suit (i.e., a bid
of \he{2}/\sp{2}), it shows the unbid major and an unspecified minor.
